...
class LabelNestsApp(QtWidgets.QWidget):
    ...
    def __init__(self, folder_path, start_image=None):
        ...
        self.setWindowTitle('LabelNests Annotation Helper')
        ...
        if os.path.exists(json_path) and os.path.exists(annotated_path):
            ...
            if json_timestamp > annotated_image_timestamp:
                if not self.viewing_annotated_current:
                    ...
                    f"⚠️ {image_name}\nWARNING: Annotations exist but the annotated image might not be up to date.\nJSON last saved on: {json_timestamp}\nAnnotated image generated on: {annotated_image_timestamp}"
                )
                else:
                    self.message_label.setText(
                ...
                self.message_label.setText(
                    f"✅ {image_name}\nAnnotations already exist. JSON last saved on: {json_timestamp}\nAnnotated image generated on: {annotated_image_timestamp}"
                )
    ...
    def update_image(self):
        ...
        if os.path.exists(json_path):
            ...
            if os.path.exists(annotated_path):
                ...
                if json_timestamp > annotated_image_timestamp:
                    self.message_label.setText(
                    f"⚠️ WARNING: Annotations already exist for {image_name}, but the JSON is newer than the annotated image. (JSON last saved on: {json_timestamp})\nAnnotated image generated on: {annotated_image_timestamp}"
                )
                else:
                    self.message_label.setText(
                        f"✅ Annotations already exist: {image_name} (JSON last saved on: {json_timestamp})\nAnnotated image generated on: {annotated_image_timestamp}"
                    )
            else:
                self.message_label.setText(
                    f"✅ Annotations already exist: {image_name} but no annotated image has been generated (JSON last saved on: {json_timestamp})"
                )
        else:
            if self.current_index > 0:
                prev_name = self.image_list[self.current_index - 1]
                ...
                if os.path.exists(prev_json):
                    ...
                    self.autoloaded_data = data
                    self.message_label.setText(
                        f"📋 Current: {image_name} | Will load annotations from {prev_name} in LabelMe upon launch (not yet saved)"
                    )
                else:
                    self.message_label.setText(
                        f"📋 Current: {image_name} | No annotations found"
                    )
            else:
                self.message_label.setText(
                    f"📋 Current: {image_name} | No annotations found"
