...
def PropInactiveTime(broodLR):
    ...
    work = broodLR[[col for col in broodLR.columns
                    if col[0].startswith("distM_")
                    and any(label in col[0].lower() for label in
                            ("nest perimeter", "egg", "larv", "pup", "wax", "nectar", "pollen"))]]
