    ...
    for name, rawOneLR in datasets:
        ...
        for test in funcs:
            try:
                ...
                # Handle functions that return both (summary, frame_vector_df)
                if isinstance(result, tuple) and len(result) == 2:
                    summary_result, frame_vectors = result

                    # Append frame-level data to oneLR (columns must not conflict)
                    if not frame_vectors.empty:
                        oneLR = pd.concat([oneLR, frame_vectors], axis=1)

                    # Convert summary result to Series
                    if isinstance(summary_result, (np.ndarray, list)):
                        summary_result = pd.Series(summary_result, index=ordered_bee_ids)

                    analysis[test[0]] = summary_result

                # Fallback for old single-value-returning functions
